test_that("execution fails if there are no facets in the plot", {
  expect_error(reveal_by_panel(make_test_plot("nopanel")))
})
...
test_that("output has correct length", {
  p <- make_test_plot()
  expect_length(reveal_by_panel(p), 10)
  expect_length(reveal_by_panel(p, order = -1), 9)
...
test_that("plots look like they should - only data",{
  plot_list <- reveal_by_panel(make_test_plot(), order = c(2,1,3))
...
test_that("plots look like they should - everything",{
  plot_list <- reveal_by_panel(make_test_plot(), order = c(2,1,3), "everything")
