test_that("execution fails if there are no groups in the plot", {
  expect_warning((reveal_by_group(make_test_plot("nogroup"))))
})
...
test_that("output has correct length", {
  p <- make_test_plot()
  expect_length(reveal_by_group(p), 4)
  expect_length(reveal_by_group(p, order = -1), 3)
...
test_that("plots look like they should",{
  plot_list <- reveal_by_group(make_test_plot(), order = c(2,1,3))
