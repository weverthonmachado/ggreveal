...
- order:
  ...
  Any panel not included in the vector will be omitted from the
  incremental plots. E.g.: with `order = c(3, 1)`, the second panel is
  not shown.
...
- what:
  ...
  (optional) one of `"data"` or `"everything"`.' With `"data"` (the
  default), the basic graph layout, including axes and facet labels, is
  shown from the start, and only the data points are shown
  incrementally. With `"everything"`, the entire panels are shown
  incrementally.
...
#> 
#> ✔ /tmp/RtmpIa3asK/myplot_0.png
#> ✔ /tmp/RtmpIa3asK/myplot_1.png
#> ✔ /tmp/RtmpIa3asK/myplot_2_last.png
